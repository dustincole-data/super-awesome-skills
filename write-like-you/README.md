...
before and after, works out the durable rule behind your change, and (only after you
approve it) folds that rule back into your voice guide. It gets closer to you every time
you use it.

One writes. The other learns.
...
That is the whole idea. Your writing already contains your voice. This just points the
model at it.

## How to use it

1. **Drop both folders into your agent's skills directory** (Claude Code, Codex, anything
   that reads a `SKILL.md`).
2. **Fill in `write-like-you` once.** Grab 5 to 10 things you actually wrote and liked.
   Paste real quotes as the examples. List the words that are never yours. The skill walks
   you through it and tells you to cut any rule you can't back with a real sentence.
...
This will not sound like you on day one. It sounds like you after you have fed it a handful
of your own edits, because that is when it finally has enough of you to copy.

And a voice guide is only as honest as the writing you build it from. Garbage in, confident
garbage out.

I built this for my own writing and I am publishing the generic version because the method
works for anyone who writes. If it helps, steal it.
